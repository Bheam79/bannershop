...
- **BANNERSH-127: OpenAI key precedence is DB > appsettings.** `OpenAiImageService.GetEffectiveApiKeyAsync` reads `system_settings.openai_api_key` first (set via admin panel) and falls back to `OpenAi:ApiKey` from config. If an old admin-panel value is in the DB, it will mask any change made to `appsettings.Local.json`. The startup log (`Startup.OpenAi` category) now enumerates each configuration provider's contribution AND probes the DB row so the resolved source is visible in journalctl on every boot.
