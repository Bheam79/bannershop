...
```
Current DB IP: `10.89.8.2` (changes each time the container restarts — re-read with the `DB_IP=$(cb docker exec db hostname -I | tr -d ' ')` snippet above; appsettings.json is hard-coded to a previous IP and may need updating).
...
- **BANNERSH-289: fal.ai image generation.** `FalAiImageService` is the registered `IAiImageService` and calls `fal-ai/flux-2-pro` (or `/edit` with a portrait); `Fal:ModelId`/`BaseUrl` are non-secret appsettings and `system_settings.fal_api_key` is the DB-only secret. OpenAI remains only for optional prompt refinement.
- **BANNERSH-290: portrait references.** fal.ai edit requests always name the attached portrait as `@image1` in the prompt. Anonymous portrait uploads are resolved by `(BannerDesign.Id, IpAddress)` and carried into the design request; otherwise the pipeline would silently fall back to text-to-image despite showing an uploaded-photo preview.
...
- **BANNERSH-127 (historical):** `OpenAiImageService` supported DB-first image model/quality overrides. BANNERSH-289 replaced it as the registered image provider and removed those two settings; `openai_api_key` remains for prompt refinement.
