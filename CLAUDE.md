...
- **BANNERSH-293: Claude OAuth lifecycle.** `/admin/settings` can run the Claude Code PKCE copy/paste flow; access/refresh/expiry values stay in masked `system_settings` rows and `ClaudeOAuthRefreshService` rotates them before expiry. Saving a manual setup-token clears prior refresh metadata. Claude credentials are always seeded blank; `CLAUDE_CODE_OAUTH_TOKEN` remains the environment fallback.
