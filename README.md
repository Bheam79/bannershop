...
- Claude Code CLI (for vivid image prompt refinement)
...
details. Claude Code (Sonnet by default) then refines that prompt; if Claude is
not configured or fails, generation continues with the deterministic prompt.
Codex and Grok native image tools generate alternatives in parallel. Connect both
accounts under `/admin/settings`; executable paths and timeouts are configured
under `ImageCli`. One credit buys the pair, and a failed pair is refunded.
The older OpenAI API implementations remain only as legacy code.

The API invokes the configured `ClaudeCli:ExecutablePath` as a child process,
equivalent to:

```bash
CLAUDE_CODE_OAUTH_TOKEN=... claude \
  --print --no-session-persistence --disable-slash-commands \
  --setting-sources "" --tools "" --output-format text \
  --model sonnet --system-prompt "..."
```

The customer-derived prompt is written to standard input. The
`--no-session-persistence` flag is intentional, so these calls do **not** create
transcripts in `~/.claude/projects`. Success (prompt lengths only), skipped
refinement, and failures are logged by the API instead; in production, inspect
