...
<template>
  <div class="max-w-7xl mx-auto px-4 py-8">
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold text-gray-900">Ordrer</h1>
        <p v-if="!loading" class="text-sm text-gray-500 mt-0.5">{{ totalCount }} ordre{{ totalCount !== 1 ? 'r' : '' }} totalt</p>
      </div>
    </div>
    ...
    <!-- Error -->
    <div v-else-if="error" class="bg-red-50 border border-red-200 text-red-800 rounded-xl p-5 text-center">
      {{ error }}
    </div>

    <!-- Empty -->
    <div v-else-if="orders.length === 0" class="bg-white border border-gray-200 rounded-xl p-12 text-center text-gray-400">
      Ingen ordrer funnet.
    </div>
      ...
      <div class="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
        <table class="w-full text-sm">
          <thead class="bg-gray-50 border-b border-gray-200">
            <tr>
              <th class="text-left px-4 py-3 font-medium text-gray-600">Ordre #</th>
              <th class="text-left px-4 py-3 font-medium text-gray-600">Kunde</th>
              <th class="text-left px-4 py-3 font-medium text-gray-600">Dato</th>
              <th class="text-left px-4 py-3 font-medium text-gray-600">Status</th>
              <th class="text-left px-4 py-3 font-medium text-gray-600 hidden md:table-cell">Levering</th>
              <th class="text-left px-4 py-3 font-medium text-gray-600 hidden lg:table-cell">Varer</th>
              <th class="text-right px-4 py-3 font-medium text-gray-600">Totalt</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
            ...
            >
              ...
              <td class="px-4 py-3 text-gray-500 hidden md:table-cell">{{ deliveryLabel(order.deliveryType) }}</td>
              <td class="px-4 py-3 text-gray-500 hidden lg:table-cell">{{ order.itemCount }} stk</td>
              <td class="px-4 py-3 text-right font-semibold text-gray-900">{{ formatNok(order.totalNok) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
