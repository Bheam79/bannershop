...
import { orderStatusLabel as statusLabel, orderStatusAdminClass as statusClass } from '@/utils/orderStatus'
...
<template>
  <div class="max-w-7xl mx-auto px-4 py-8">
    <div class="mb-8">
      ...
      <p class="text-gray-400 mt-1">Innlogget som <strong>{{ auth.user?.name }}</strong></p>
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      <div class="bg-gray-800 rounded-xl border border-gray-700 p-5">
        ...
        <div class="text-3xl font-bold text-gray-100">
          <span v-if="kpiLoading" class="text-gray-600">—</span>
    ...
    <div class="bg-gray-800 rounded-xl border border-gray-700 mb-8">
      <div class="flex items-center justify-between px-5 py-4 border-b border-gray-700">
        <h2 class="text-base font-semibold text-gray-100">Siste 10 ordrer</h2>
        <RouterLink to="/admin/orders" class="text-sm text-blue-400 hover:underline">Se alle →</RouterLink>
      </div>

      <div v-if="kpiLoading" class="px-5 py-8 text-center text-gray-500">Laster…</div>
      <div v-else-if="recentOrders.length === 0" class="px-5 py-8 text-center text-gray-500">Ingen ordrer ennå.</div>
      <table v-else class="w-full text-sm">
        <thead class="bg-gray-900 border-b border-gray-700">
          <tr>
            <th class="text-left px-5 py-3 font-medium text-gray-400">Ordre #</th>
            <th class="text-left px-5 py-3 font-medium text-gray-400">Kunde</th>
            <th class="text-left px-5 py-3 font-medium text-gray-400">Dato</th>
            <th class="text-left px-5 py-3 font-medium text-gray-400">Status</th>
            <th class="text-right px-5 py-3 font-medium text-gray-400">Totalt</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-700">
          ...
          >
            <td class="px-5 py-3 font-medium text-blue-400">#{{ order.id }}</td>
            <td class="px-5 py-3 text-gray-200">
              <div class="font-medium">{{ order.customerName ?? '—' }}</div>
              <div class="text-xs text-gray-500">{{ order.customerEmail }}</div>
            </td>
            <td class="px-5 py-3 text-gray-400">{{ formatDate(order.createdAt) }}</td>
            <td class="px-5 py-3">
              <span class="text-xs font-semibold px-2 py-0.5 rounded-full" :class="statusClass(order.status)">
                {{ statusLabel(order.status) }}
              </span>
            </td>
            <td class="px-5 py-3 text-right font-semibold text-gray-100">{{ formatNok(order.totalNok) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      >
        ...
        <h2 class="font-semibold text-gray-100">Ordrer</h2>
        <p class="text-sm text-gray-400 mt-1">Se og administrer alle kundeordrer</p>
      </RouterLink>
      <RouterLink
        to="/admin/sizes"
        class="block bg-gray-800 rounded-xl border border-gray-700 p-6 hover:border-blue-500 hover:bg-gray-700 transition"
      >
        <div class="text-3xl mb-3">📐</div>
        <h2 class="font-semibold text-gray-100">Bannerstørrelser</h2>
        <p class="text-sm text-gray-400 mt-1">Administrer størrelser og priser</p>
      </RouterLink>
      <RouterLink
        to="/admin/materials"
        class="block bg-gray-800 rounded-xl border border-gray-700 p-6 hover:border-blue-500 hover:bg-gray-700 transition"
      >
        <div class="text-3xl mb-3">🧵</div>
        <h2 class="font-semibold text-gray-100">Materialer</h2>
        <p class="text-sm text-gray-400 mt-1">Administrer banner-materialer</p>
      </RouterLink>
      <RouterLink
        to="/admin/pricing"
        class="block bg-gray-800 rounded-xl border border-gray-700 p-6 hover:border-blue-500 hover:bg-gray-700 transition"
      >
        <div class="text-3xl mb-3">💰</div>
        <h2 class="font-semibold text-gray-100">Prissetting</h2>
        <p class="text-sm text-gray-400 mt-1">Juster prisparametere</p>
      </RouterLink>
      <RouterLink
        to="/admin/users"
        class="block bg-gray-800 rounded-xl border border-gray-700 p-6 hover:border-blue-500 hover:bg-gray-700 transition"
