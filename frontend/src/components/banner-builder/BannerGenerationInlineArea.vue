...
<template>
  ...
  <div style="margin-top:20px">
    ...
    <div v-else-if="genPhase === 'error'" class="preview-generating preview-error-frame">
      ...
      <p v-else-if="['openai_quota_exceeded', 'fal_quota_exceeded', 'image_providers_failed'].includes(currentDesignRequest?.lastError ?? '')" style="font-size:13.5px;color:var(--muted);text-align:center;max-width:26em">
    ...
    <div v-if="genPhase !== 'submitting' && genPhase !== 'generating' && genPhase !== 'anon_pending'" style="margin-top:16px;display:grid;gap:12px">
      ...
      <button
        ...
        :disabled="isManual ? (manualSubmitting || !step2Valid) : (approving || activatingGenerationId !== null)"
        @click="emit('proceed')"
      >
        <i v-if="isManual ? manualSubmitting : (approving || activatingGenerationId !== null)" class="fa-solid fa-circle-notch fa-spin"></i>
